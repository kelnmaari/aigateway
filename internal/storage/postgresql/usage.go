...
func (db *PostgreSQLDB) GetUserUsageStats(ctx context.Context, userID string, period time.Duration) (*models.UsageStats, error) {
	...
	err := db.db.QueryRowContext(ctx, query, userID, startTime).Scan(
		&stats.TotalRequests,
	...
	rows, err := db.db.QueryContext(ctx, modelQuery, userID, startTime)
	if err != nil {
	...
	rows, err = db.db.QueryContext(ctx, endpointQuery, userID, startTime)
	if err != nil {
	...
	rows, err = db.db.QueryContext(ctx, apiKeyQuery, userID, startTime)
	if err != nil {
		db.logger.WithError(err).Warn("Failed to get API key usage")
	} else {
		defer rows.Close()
		...
		for rows.Next() {
			...
			if err := rows.Scan(&apiKey.KeyID, &apiKey.Requests, &apiKey.Tokens, &lastUsedStr); err != nil {
				db.logger.WithError(err).Warn("Failed to scan API key usage")
				continue
			}

			// Parse timestamp with multiple format support
			if lastUsedStr.Valid && lastUsedStr.String != "" {
				apiKey.LastUsed = db.parseTimestamp(lastUsedStr.String)
			} else {
	...
	rows, err = db.db.QueryContext(ctx, recentQuery, userID, startTime)
	if err != nil {
		db.logger.WithError(err).Warn("Failed to get recent requests")
	} else {
		defer rows.Close()
		...
		for rows.Next() {
			...
			if err := rows.Scan(&timestampStr, &req.Model, &req.KeyID, &req.Tokens, &req.Duration, &req.Success); err != nil {
				db.logger.WithError(err).Warn("Failed to scan recent request")
				continue
			}

			// Parse timestamp with multiple format support
			if timestampStr.Valid && timestampStr.String != "" {
				req.Timestamp = db.parseTimestamp(timestampStr.String)
			} else {
...
func (db *PostgreSQLDB) parseTimestamp(ts string) time.Time {
	...
	if idx := strings.Index(ts, " m="); idx > 0 {
		cleaned := ts[:idx]
		return db.parseTimestamp(cleaned) // Рекурсивно с очищенной строкой
	}

	db.logger.Warnf("Failed to parse timestamp in any format: %s", ts)
...
func (db *PostgreSQLDB) GetTenantUsageStats(ctx context.Context, tenantID string, period time.Duration) (*models.UsageStats, error) {
	...
	err := db.db.QueryRowContext(ctx, query, tenantID, startTime).Scan(
		&stats.TotalRequests,
		&stats.SuccessfulRequests,
		&stats.FailedRequests,
		&stats.PromptTokens,
	...
	rows, err := db.db.QueryContext(ctx, modelQuery, tenantID, startTime)
	if err != nil {
		return nil, fmt.Errorf("failed to get model usage: %w", err)
	}
	defer rows.Close()
	...
	rows, err = db.db.QueryContext(ctx, endpointQuery, tenantID, startTime)
	if err != nil {
		return nil, fmt.Errorf("failed to get endpoint usage: %w", err)
	}
	defer rows.Close()
...
`
	...
	err := db.db.QueryRowContext(ctx, query, start, end).Scan(
		&stats.TotalRequests,
		&stats.SuccessfulRequests,
		&stats.FailedRequests,
		&stats.TotalTokens,
...
`
	...
	rows, err := db.db.QueryContext(ctx, modelQuery, start, end)
	if err != nil {
...
`
	...
	rows2, err := db.db.QueryContext(ctx, userQuery, start, end)
	if err == nil {
		defer rows2.Close()
		for rows2.Next() {
			var user struct {
...
`
	...
	rows, err := db.db.QueryContext(ctx, percQuery, start, end)
	if err == nil {
		defer rows.Close()
		durations := []float64{}
		for rows.Next() {
	...
	slowQuery := `SELECT COUNT(*) FROM api_usage WHERE created_at >= $1 AND created_at <= $2 AND duration_ms > 5000`
	db.db.QueryRowContext(ctx, slowQuery, start, end).Scan(&stats.SlowRequestsCount)
