...
func (db *PostgreSQLDB) CreatePermission(ctx context.Context, permission *models.RBACPermission) error {
	...
	db.logger.WithField("permission_id", permission.ID).Info("Permission created")
	return nil
}
...
func (db *PostgreSQLDB) CreateRole(ctx context.Context, role *models.Role) error {
	...
	db.logger.WithField("role_id", role.ID).Info("Role created")
	return nil
}
...
func (db *PostgreSQLDB) GetRoleByName(ctx context.Context, name string, tenantID *string) (*models.Role, error) {
	...
	query := `
		SELECT id, name, display_name, description, type, scope, tenant_id, created_at, updated_at
		FROM roles
		WHERE name = $1 AND (tenant_id = $2 OR (tenant_id IS NULL AND $3 IS NULL))
	`

	var role models.Role
	err := db.db.QueryRowContext(ctx, query, name, tenantID, tenantID).Scan(
		&role.ID,
		&role.Name,
		&role.DisplayName,
		&role.Description,
...
func (db *PostgreSQLDB) UpdateRole(ctx context.Context, role *models.Role) error {
	...
	db.logger.WithField("role_id", role.ID).Info("Role updated")
	return nil
}
...
func (db *PostgreSQLDB) DeleteRole(ctx context.Context, id string) error {
	...
	db.logger.WithField("role_id", id).Info("Role deleted")
	return nil
}
...
func (db *PostgreSQLDB) AssignPermissionToRole(ctx context.Context, roleID, permissionID string) error {
	...
	_, err := db.db.ExecContext(ctx, query, roleID, permissionID)
	if err != nil {
		return fmt.Errorf("failed to assign permission to role: %w", err)
	}

	db.logger.WithField("role_id", roleID).WithField("permission_id", permissionID).Info("Permission assigned to role")
	return nil
}
...
func (db *PostgreSQLDB) RemovePermissionFromRole(ctx context.Context, roleID, permissionID string) error {
	...
	db.logger.WithField("role_id", roleID).WithField("permission_id", permissionID).Info("Permission removed from role")
	return nil
}
...
func (db *PostgreSQLDB) GetRolePermissions(ctx context.Context, roleID string) ([]*models.RBACPermission, error) {
	...
	rows, err := db.db.QueryContext(ctx, query, roleID)
	if err != nil {
...
func (db *PostgreSQLDB) AssignRoleToUser(ctx context.Context, userRole *models.UserRole) error {
	...
	db.logger.WithField("user_id", userRole.UserID).WithField("role_id", userRole.RoleID).Info("Role assigned to user")
	return nil
}
...
func (db *PostgreSQLDB) RemoveRoleFromUser(ctx context.Context, userID, roleID string, tenantID *string) error {
	...
	query := `
		...
		WHERE user_id = $1 AND role_id = $2 AND (tenant_id = $3 OR (tenant_id IS NULL AND $4 IS NULL))
	`

	_, err := db.db.ExecContext(ctx, query, userID, roleID, tenantID, tenantID)
	if err != nil {
		return fmt.Errorf("failed to remove role from user: %w", err)
	}

	db.logger.WithField("user_id", userID).WithField("role_id", roleID).Info("Role removed from user")
	return nil
}
