...
// CreateQualityIssue POST /api/admin/gitlab/projects/:id/quality/create-issue
func (h *GitLabQualityHandler) CreateQualityIssue(c *gin.Context) {
	projectID := c.Param("id")
	if projectID == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Project ID is required"})
		return
	...
	var req CreateQualityIssueRequest
	if err := c.ShouldBindJSON(&req); err != nil {
...
func (h *GitLabQualityHandler) createGitLabIssue(ctx context.Context, gitlabURL, token string, projectID int64, req CreateQualityIssueRequest) (string, error) {
	h.logger.WithFields(logrus.Fields{
		...
		"title":             req.Title,
	}).Info("Creating GitLab issue for quality")
	...
	labels := req.Labels
	if len(labels) == 0 {
		labels = []string{"code-quality", "improvement"}
	}
