...
type VectorDocument struct {
	...
	Metadata  map[string]any // Метаданные (source_id, document_id, etc.)
...
type SearchRequest struct {
	...
	Filters        map[string]any // Фильтры по метаданным
...
type VectorStore interface {
	...
	DeleteByMetadata(ctx context.Context, filters map[string]any) (int, error)
	...
	CreateIndex(ctx context.Context, indexType string, params map[string]any) error
...
type PostgreSQLConfig struct {
	...
	// Index settings
	HNSWParams map[string]any // m, ef_construction, ef_search
}
