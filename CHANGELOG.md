...
### Added
- **Edit Saved Model**: Target Node dropdown в модальном окне редактирования сохранённой модели — можно переназначить модель на другой воркер
- **UpdateSavedRequest**: поле `target_node` для сохранения привязки модели к воркеру
...
### Fixed
- **Chat UI**: исправлен парсинг `<think>` блоков — старый regex с ленивым квантификатором ломался на multiline markdown-контенте (заголовки, bold, списки внутри thinking). Заменён на двухшаговый поиск по индексам: find open tag → find close tag в остатке → extract between
...
- **`inference_proxy_handler`: preserve all request fields**: Switched from struct-unmarshal+re-marshal to raw-body pass-through, so provider-specific fields (`thinking_budget`, etc.) are no longer silently dropped.

### Technical

- `internal/api/handlers/message_converter.go` — new file: `normalizeMessagesInJSON`, `convertAssistantMessage`, `convertToolResultMessages`, `extractToolResultContent`
...
- `internal/inference/service.go`, `manager.go`, `router.go` — plumbed `OnnxExporter()` getter through the stack
...
- `web-svelte/src/lib/api/inference.ts` — `OnnxExportRequest`, `OnnxExportJob` types; `startOnnxExport`, `getOnnxExportJob`, `listOnnxExportJobs` API methods
- `web-svelte/src/routes/(protected)/admin/models/+page.svelte` — ONNX export modal with form, job polling, log viewer; ONNX button on TEI model cards
...
- **TEI CPU Mode: `--gpus all` Still Passed**: Despite selecting CPU Mode and using the `cpu-1.8` image, Docker runtime unconditionally added `--gpus all` because empty `GPUDevice` was treated as "all GPUs". CPU-only image crashed immediately when NVIDIA runtime was injected. Added `CPUOnly bool` field to `ContainerStartRequest`; when set, GPU device requests and `--gpus` flag are completely omitted from both Docker SDK and CLI paths. `--ipc=host`/`--shm-size`/`--ulimit memlock` (NCCL flags) are also skipped for CPU containers.
...
- **Deep Scan Improvements**:
  - Enhanced hallucination detection for numeric garbage patterns (I4, I5, I6, I12, I28, I64, I128)
  - Added SSE streaming mode with real-time progress updates (`stream: true` parameter)
  - Test file filtering to skip `_test.go`, `/fixtures/`, `/mocks/` directories
