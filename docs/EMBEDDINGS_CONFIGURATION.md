...
Ollama-OpenAI Proxy поддерживает **embeddings** (векторные представления) для текстов через `/v1/embeddings` endpoint. Начиная с версии **v1.10.1**, вы можете настроить **дефолтную модель** для embeddings в конфигурации.
