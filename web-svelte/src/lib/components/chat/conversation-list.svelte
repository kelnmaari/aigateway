<script lang="ts">
	...
	import { faComments, faPlus, faTrash, faEllipsisVertical } from '@fortawesome/free-solid-svg-icons';
	...
	<div class="flex-1 overflow-y-auto px-2">
		{#if conversations.length === 0}
			<div class="flex flex-col items-center justify-center py-12 text-center">
				<FontAwesomeIcon icon={faComments} class="h-10 w-10 text-muted-foreground/40" />
		...
		{:else}
			<div class="space-y-1 pb-4">
				{#each conversations as conversation (conversation.id)}
					...
					>
						<FontAwesomeIcon icon={faComments} class="h-4 w-4 shrink-0" />
